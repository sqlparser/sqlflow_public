...
public class TestGSP {
    ...
    static void test(String name, EDbVendor vendor, String sql) {
        try {
            TGSqlParser parser = new TGSqlParser(vendor);
            parser.sqltext = sql;
            int ret = parser.parse();
            if (ret == 0) {
                System.out.println("  PASS: " + name + " (" + vendor + ")");
                passed++;
            } else {
                String msg = parser.getErrormessage();
                if (msg != null && msg.length() > 120) msg = msg.substring(0, 120) + "...";
                System.out.println("  FAIL: " + name + " (" + vendor + ") — " + msg);
                failed++;
            }
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        ...
        test("Oracle MERGE with error logging", EDbVendor.dbvoracle,
        ...
        // SQL Server
        test("T-SQL CROSS APPLY", EDbVendor.dbvmssql,
        ...
        test("T-SQL TRY/CATCH stored procedure", EDbVendor.dbvmssql,
        ...
        // PostgreSQL
        test("PL/pgSQL function with RETURN QUERY", EDbVendor.dbvpostgresql,
        ...
        // Oracle PL/SQL
        test("Oracle PL/SQL with BULK COLLECT", EDbVendor.dbvoracle,
        ...
        // BigQuery
        test("BigQuery UNNEST with STRUCT", EDbVendor.dbvbigquery,
            "SELECT u.id, item.name, item.quantity FROM users u, UNNEST(u.order_items) AS item WHERE item.quantity > 5");

        test("BigQuery procedural DECLARE/IF", EDbVendor.dbvbigquery,
