...
### Grabit directory of data files instructions

- The submitted jobname is `grabit_%yyyyMMddHHmmss%`.

- After the task completes, the metadata data is generated in the `data/job_%jobname%/metadata` directory.

- When the task is completed, the analysis result file generated by the task will be stored in the `data/job_%jobname%/result` directory.
