...
### 3.5. List tables/columns in a specific job

```
/list_tables_in_job <job_name>
```
This will list all tables in the dlineage job `<job_name>`. Each table name includes its database and schema name (if present). Tables are sorted alphabetically.

list columns in a table
```
/list_columns_in_table <job_name> <table_name>
```
This will list all columns in the table `<table_name>` in the dlineage job `<job_name>`.
Columns are sorted alphabetically.

### 3.6 Export all data lineage to a csv file

```
/export_dlineage_to_csv <job_name> <output_csv_file>
```
This will export all data lineage in the dlineage job `<job_name>` to the csv file `<output_csv_file>`.
If more than 1000 records in the job, don't export the data lineage to the csv file, and tell the user to use the `/query_dlineage_downstream` or `/query_dlineage_upstream` commands to get a smaller csv file.
